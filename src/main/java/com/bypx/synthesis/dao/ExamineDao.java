...
public class ExamineDao {
    ...
    public List query(InputParam param) {
        ...
        String sql="SELECT id_,type_id_,number_,price_,creater_,creater_id_,order_type_,status_,DATE_FORMAT(create_date_,'%Y-%m-%d') AS create_date_  FROM jxc_product_order WHERE status_=1 ";
        sql+=formatSql(param);
    ...
    public void add(Outbound outbound) {
        String sql = " insert into jxc_product_order(id_,type_id_,number_,price_,order_type_,status_,creater_,creater_id_,create_date_) values(?,?,?,?,?,?,?,?,?) ";
        jdbcTemplate.update(sql,outbound.getId(),outbound.getTypeId(),outbound.getNumber(),outbound.getPrice(),outbound.getOrderType(),outbound.getStatus(),outbound.getCreator(),outbound.getCreatorId(),outbound.getCreateTime());
    }
